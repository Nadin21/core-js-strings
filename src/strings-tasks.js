...
function removeFirstOccurrences(str, value) {
  const startDel = str.indexOf(value);
  if (startDel === -1) return str;
  const endDel = value.length + startDel;
  return str.substring(0, startDel).concat(str.substring(endDel));
}
...
function removeLastOccurrences(str, value) {
  const startDel = str.lastIndexOf(value);
  if (startDel === -1) return str;
  const endDel = value.length + startDel;
  return str.substring(0, startDel).concat(str.substring(endDel));
}
...
function countVowels(str) {
  ...
  const strVowels = str.split('').filter((item) => vowels.includes(item));
...
function isPalindrome(str) {
  ...
  for (let i = 0; i < Math.floor(arrCharLength / 2); i += 1) {
    if (arrChar[i] !== arrChar[arrCharLength - 1 - i]) return false;
  }
  return true;
}
...
function findLongestWord(sentence) {
  const arrayWords = sentence.split(' ').sort((a, b) => b.length - a.length);
  return arrayWords[0];
}
...
function encodeToRot13(str) {
  const inputChar = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const outputROT13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  const encode = (char) =>
    inputChar.includes(char) ? outputROT13[inputChar.indexOf(char)] : char;
